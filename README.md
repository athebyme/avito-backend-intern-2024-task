...
#### 2. Проверка доступности сервера Ping
- **Эндпоинт:** GET /ping
...
- **Описание:** Метод возвращает список доступных тендеров с возможностью фильтрации по типу услуг.
...
- `authorUsername` (обязательный): Имя автора, отправившего предложение на тендер компании.         - Query / String
- `requesterUsername` (обязательный): Имя ответственного от организации лица.         - Query / String
- `limit` (опционально, по умолчанию: 5): Ограничивает количество возвращаемых предложений.         - Query / int32
- `offset` (опционально, по умолчанию: 0): Указывает смещение для получения последующих записей.         - Query / int32
