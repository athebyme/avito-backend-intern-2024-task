...
Для того чтобы запустить приложение в контейнере Docker:
1. отключите k8s работающий pod
2. kubectl rollout restart deployment cnrprod1725773447-team-79619-backend --namespace=cnrprod1725773447-team-79619
(p.s. я не знаю почему, но похоже на базе есть лимит подключений. приложение невозможно запустить, пока работает под, поскольку не создается бин Hikari драйвера)
...
- **Например:** User3 создал тендер. User2 решил отправить предложение на этот тендер. User3 отправляет отзыв на это предложение. || Теперь тендер создает User1. User2 также создал предложение на этот тендер. User1 может этим запросом посмотреть отзыв User3 на предложение User2 по его тендеру.
